...
{foreach key=row item=cat from=$subs}
    ...
    {if $settings.sort_override == 1}
      <tr>
        <td colspan="4" align="center">
          <form action="index.php?act=category&id={$id}&sort" method="post">
          {#sort_by#}: 
          <select name="sortby">
...
<a href="{$settings.dburl}/index.php?act=category&amp;id={$id}&amp;start={$pages[pages].1}">{if $pages[pages].0 eq "back"}&laquo;&laquo;{elseif $pages[pages].0 eq "forward"}&raquo;&raquo;{else}{$pages[pages].0}{/if}</a>
{/section}
</div><p />
{/if}